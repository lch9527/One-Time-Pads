...
char * encryption(char * text_key){
  char* token = strtok(text_key, "$");
    ...
    char *dephertext = (char*)malloc( strlen(token) +1);
    strcpy(text,token);
    token = strtok(NULL, "$");
    char* key = (char*)malloc( strlen(token) +1);
    strcpy(key,token);
	
    long int uc_num, c_num, k_num;
    char c_chat; 
   // printf("text : %s key : %s", text, key);
   
    for (int i = 0; i < strlen(text); i++){
        c_num = to_num(text[i]);
        k_num = to_num(key[i]);
        uc_num = c_num - k_num;
        c_chat = to_char(uc_num);
        //printf("c_num : [%d], k_num :[%d], uc_num :[%d], chat :[%s]\n", c_num, k_num, uc_num, c_chat);
        dephertext[i] = c_chat;
    }
    //printf("%s", dephertext);
    return dephertext;
...
memset(buffer, '\0', 256);
    ...
    charsWritten = send(connectionSocket, "dec_server", 11, 0); 
    if (charsWritten < 0){
      error("ERROR writing to socket");
    }
close(listenSocket);
